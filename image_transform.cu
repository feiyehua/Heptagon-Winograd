  ...
  for (int64_t h = 0; h < ti.tile_in_h; ++h) {
    for (int64_t w = 0; w < ti.tile_in_w; ++w) {
      ...
      device_packed_image_tensor[x_index * device_packed_image->ysize * device_packed_image->pitch /
                                     sizeof(float) +
                                 h * device_packed_image->pitch / sizeof(float) +
                                 w] = device_image_tensor[(batch * blockDim.x + ic) * device_image->ysize *
                                                              device_image->pitch / sizeof(float) +
                                                          h * device_image->pitch / sizeof(float) + w];
    }
  }
}
// get V tensor = BT*d*B
__global__ void image_transform(const cudaPitchedPtr *__restrict__ device_packed_image,
                                const cudaPitchedPtr *__restrict__ V,
                                const V_shape_t vs,
                                const tiling_info_t ti,
...
void device_image_transform(float *__restrict__ image,
  ...
  cudaExtent extent = make_cudaExtent(
      sizeof(float) * is.bs * is.ic,
      ti.tiles_on_h * ti.tile_in_h,
      ti.tiles_on_w * ti.tile_in_w);  // 分配足够的内存，去掉image_packing中的分支
  cudaMalloc3D(&device_image, extent);
  cudaExtent image_extent = make_cudaExtent(sizeof(float) * is.bs * is.ic, is.h, is.w);
  ...
  cudaExtent device_packed_image_extent = make_cudaExtent(
      sizeof(float) * ti.num_tiles * is.ic, ti.tile_in_h, ti.tile_in_w);
  cudaMalloc3D(&device_packed_image, extent);
  ...
  cudaExtent V_tensor_extent = make_cudaExtent(
      sizeof(float) * ti.tile_in_h, ti.tile_in_w, vs.ic * vs.num_tiles);
